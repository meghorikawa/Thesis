...
of a form across proficiency and accuracy, which assess the learber's ability to use a form correctly(in comparison
...
begin to produce relative clauses at lower levels, but only achieve an accurate useage at the upper-intermediate
levels.
...
feature, these features also match the frequency of use used by native speakers.
Negative features on the other hand, are features which are absent or persistent errors at the lower levels but
disappear with development as learners master a form, these features also do not match the frequency of use by native
speakers.
...
high proficiency levels - falling in line with

\citet{salamoura2010} also working within the English Profile Programme, analyzed learner texts in the Cambridge
Learner Corpus (
CLC) to identify features distinguishing between CEFR proficiency levels. Their findings support those of
\citet{Hawkins_Buttery_2010}, observing that learner errors decrease and L1 influence weakens at the higher proficiency levels.

The above findings not only support the value of criterial features as developmental indicators, but also
demonstrate the need to systemize them across proficiency levels. This has motivated profiling efforts aligned with
the standardized frameworks such as the Common European Framework of Reference (CEFR).

%need to add spainish article findings and should probably mention if any forms were found in Saroosh's article.
...
proficency assessment frameworks, which provide structured levels for describing and assessing a learner's language
ability. For indo-european languages the Common European Framework of Reference for languages (CEFR) is the most
widely adopted framework, which offers six proficency levels. Illustrative
descriptors are given for each level to illustrate what learners can achieve in communicative competence at each level.

English Grammar Profile and CEFR-J
