...
diversity of a learner's vocabulary. Researchers have operationalized lexical complexity through a range of 
text-internal measures, which are derived entirely from the learner's text, and text-external measures, which rely 
on reference corpora to assess lexical frequency or sophistication \citep{Butle2012}.

Text internal measures focus on the diversity and structure of the vocabulary used. One of the earliest and most
basic measures is the Type Token Ratio (TTR) introduced by \citet{Templin1957}. TTR measures the porportion of
unique word types to the total number of tokens in a text. However, since TTR has been found to be sensitive to text
length
\citep{koizumi2012}, more robust alternatives have been proposed, such as the correct type token ratio (CTTR)
\citep{Carroll1964}
and the Measure of Textual Lexical Diversity (MTLD) \citep{McCarthy2010}. MTLD calculates the average length of word
sequences that maintain a certain TTR value and is therefore less affected by text length, making it a suitable
measure to analyze learner texts which may vary significantly in length between proficiency levels. Other measures
such as
mean word length, used as a proxy for lexical sophistication
\footnote{ While correlation has been found between word length and sophistication (with the idea that longer words tend to be less frequent) there are many limitations in using this measure to represent lexical sophistication.}, and lexical density, which calculates the proportion
of content words (i.e. nouns, verbs, adjectives, and adverbs) in a text in a text,

Text external measures draw on frequency data from reference corpora to assess lexical sophistication-which aims
to cound the quality or "rarity" of words rather than density or diversity. Beyond evaluating the rarity of words
used in a text, the use of domain specific vocabulary can also be analyzed. One
example
of a lexical sophistication measure is the Lexical Frequency Profile introduced by \cite{Laufer1995}. The proportion
of low-frequency or advanced word mapping lexical items to known frequency bands.
