...
Syntactic complexity  is concerned with quantifying
    how elaborate and varied the grammatical structures used in a text are. A text
containing more elaboration usually contains more parts, and is consequently longer, therefore length measures are the
standard measures used.  While variation focuses on the diversity of the forms and structures used. \citet{Butle2012} further breaks syntactic measures into global and local levels of
analysis.

Global
measures concern the macro level of syntactic constructions or \textit{"the size, breadth, width, or richness of the
learner's L2 system"} \cite{Butle2012}. Measures such as mean length of T-unit and Sentence or avergae clause count
per sentence would be an example of structural patterns that reflect a learner's ability to construct more advanced
and hierarchical syntax. In contrast, the local level concerns itself with more fine-grained measures by analyzing
the internal structure at the phrase or clause level. Examples include length of noun phrases, modifiers per NP, and
depth measures such as Mean hierarchical distance (MHD).

Absolute syntactic complexity measures are some of the most widely researched measures and therefore have a long
history. Given this, .....
..T-unit vs. Sentence

Newer history using MDD measures?
