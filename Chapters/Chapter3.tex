...
features were extracted from the text and subsequently normalized (per 100 tokens) to account for variations in
...
patterns.
%%% (Total Frequency / Total tokens)* 100 normalization


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{EBMS}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
EBM are well-suited for langauge development research because of their ability to capture nonlinear effects in a
transparent and interpretable way. certain features may not increase linearly, but may rise sharply and beginning
leves, and then plateau, decrease etc.

Dynamic system theory????%refresh my memory on this aritcle.

Ebm- do not assume constant relationships..

Interpret ml package used - train interpretable glassbox models.

remove identifying information i.e. loc, participant name, etc

keep some variables like L1, gender? to check for effect - these variables ended up dominating first results so I am
running again without them.

participants language was removed due to

Native group was removed?

balance dataset(keep proportions across folds) via oversampling

Stratified 5-fold cross validation

over sampling of smaller classes within each fold
