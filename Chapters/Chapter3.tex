...
size of 50. These measures estimate the lexical diversity of morphemes are are relatively robust against text
length, making them suitable for large-scale learner corpora.
...
Calculation of the MCI is done by diving the text into consecutive segments of a fixed size (usually 5 or 10 tokens), and for each segment, a type token ratio (TTR) is calculated based on the number of unique surface forms in
