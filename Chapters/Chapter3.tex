...
the
breakdown of forms for each level can be seen in
\ref{tab:CF-Counts}
...
to be used with more complex functions or collocations at higher levels. The particle 「しか」(shika) is typically
introduced at N4 where it commonly appears with nouns to convey "only" or "nothing but" (e.g., 「学生しかいない。」
\textit{There are only students}). However, at N3, its useage often expands to combine other negative verb forms,
indicating a more advanced grammatical pattern. While the above change in useage is clearly reflected in surface
form this isn't always the case for all the grammar forms.


Another example is the phrase「ことがる」(koto ga aru) which
first
appears at the N4 level where it is used for the speaker to state they have done something, or experienced something
before. The usage expands
from N3 and
the upper levels to describe hypothetical,emphatic uses which are highly nuanced, but the surface expression does
not change in its use as can be seen below.
...
Consequently as the form does not change across uses it is difficult to distinguish between the different uses. 



In this case using the rule based matching system from spacy would be able to
differentiate between these grammar points on form alone
...
define patterns for each target grammar form.
...
features, and dependency relations within the sentence.
...
Each of the 154 criterial features was translated into one or more such rule patterns. Raw counts of the criterial
features were first extracted from the text for futher statistical analyses to examine the frequency and
distribution of the criterial features across proficency groups to identify developmental patters.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{EBMS}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Mention more details about training, how was data further prepared? about package used etc.
